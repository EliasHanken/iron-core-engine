...
anchors become solid lit cubes. After this milestone the Strandbound rope tool
*looks* like a game, not a debug view.
...
- `RopeTool` draws ropes as combined textured tube meshes and anchors as a
  combined cube mesh, through the existing lit render path.
...
- No rope endcaps — rope ends sit inside the anchor cubes.
...
- No anchor texturing — anchors are flat-colour markers (clarity over detail).
- No new gameplay.
...
For each point it places a ring of `sides` vertices (low-poly, ~6–8) in the
plane perpendicular to the local rope direction, gives each an outward-pointing
...
face) with per-face normals and simple per-face UVs, matching how `makeCube`
is built. Lets anchor cubes accumulate into one `MeshData`.
...
- `anchorTexture_` — a 1×1 solid bright-yellow texture, via `createTexture`
  ...
  a 1×1 colour tints the whole cube).
...
2. Builds one combined `MeshData` for the anchors — an `appendBox` per anchor
   — and calls `updateMesh(anchorsMesh_, ...)`.
...
games/02-strandbound/RopeTool.h, .cpp                textured tube + cube rendering (modified)
...
- `appendBox`: appending one box yields 24 vertices and 36 indices; appending
  ...
  second box references its own vertices); the box spans
  `center ± size/2`.
- `appendTube`: a tube over N points with S sides yields N×S vertices and
  `(N-1)×S×6` indices; the ring vertices sit at `radius` from the polyline;
  vertex normals point outward (away from the polyline axis); indices stay in
  range.
...
lines. Anchors are solid lit yellow cubes, clearly visible. Placing, tying, and
cutting all still work, and cut ropes disappear cleanly. The aim-marker cross
still works. `Escape` quits.
